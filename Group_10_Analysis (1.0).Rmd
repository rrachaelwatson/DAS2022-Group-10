...
final_hurdle_model_nb <- hurdle(time_at_shelter ~ animal_type + intake_type + outcome_type + chip_status, data = data10, zero.dist = "binomial", dist = "negbin")
```
...
```
The final model provides an adequate fit to the data. It has the lowest AIC of `r round(AIC(final_hurdle_model_nb), 2)` and as seen from Figure \ref{fig:final_model_rootogram} the model line fits most ot the values well.
