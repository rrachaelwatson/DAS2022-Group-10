...
month_2017 <- data10 %>% 
  ...
  select(month) %>% 
  unique()
month_2016$month %in% month_2017$month
```
...
The Poisson model is severely under-fitting zero counts. There were `r sum(data10$time_at_shelter < 1)` zero counts observed in the data set but the model only fitted `r round(exp)`. Hence a hurdle model will be fitted to provide a better fit.
...
```{r fit rootogram to check overdispersion}
#install.packages("countreg", repos="http://R-Forge.R-project.org")
library(countreg)
rootogram(hurdle_model, max = 80)
```

The model is fitting the zero counts perfectly. However, counts 1,2 and 4 are being severely under-fitted, while 6-9 are being over-fitted. There is also under-fitting at the higher counts which suggests over-dispersion. Therefore, a negative binomial hurdle model shall be fitted to address this. 
...
This shows a much better fit to the data. However, some values are still being under-fitted.
