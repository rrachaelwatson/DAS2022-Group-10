...
```
From the AIC of two models, the negative binomial model shows a much better fit to the data. However, in Figure\ref{fig:negative_binomial_hurdle_rootogram} some values are still being under-fitted.
...
Since the confidence intervals of variables animal_type and chip_status and month are not significantly different from 0, we fit the negative binomial model only depends on varables outcome_type and intake_type.
```{r}
hurdle_model_nb <- hurdle(time_at_shelter ~ . | outcome_type + intake_type, data = data10, dist = "negbin")
summary(hurdle_model_nb)
```
