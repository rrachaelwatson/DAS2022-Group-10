...
where $\hat{Y_i}$ is the expected number of days spent at the shelter, $\hat{\mu_i}$ is the mean, $n_i$ is the exposure, $\boldsymbol x_i$ denotes the matrix of covariates and $\hat{\boldsymbol \beta}$ is the matrix of coefficients. The subscript $i$ denotes the $i^{th}$ covariate pattern.
...
where $Y_i$ is the number of days spent at the shelter, $\mu_i$ is the mean and $\alpha$ is ?. The subscript $i$ denotes the $i^{th}$ covariate pattern.
